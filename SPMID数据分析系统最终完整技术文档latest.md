...
3. **MAE vs RMSE选择原则**：
   ...
   - **MAE优先**：当关注平均匹配质量和用户体验时
   - **RMSE优先**：当需要发现并消除大误差时
   - **双重评估**：实际应用中应同时使用，提供全面的质量视图
...
### 5.3 按键与锤速相关性分析 (Key vs Velocity)
...
- **性能指标汇总**：MAE、RMSE、方差、标准差等核心指标的综合展示面板
...
- **瀑布图分析**: 核心可视化界面，展示所有匹配对的质量分层，支持颜色编码
...
6. **性能指标计算**
   - 时序精度指标：MAE、RMSE、方差、标准差、95%置信区间
   - 质量分层统计：各等级分布占比和权重统计
...
| 算法  | 匹配成功率 | MAE(ms) | Excellent率 | 评判等级 |
| --- | ----- | ------- | ---------- | ---- |
| 算法A | 94.2% | 12.3    | 78.5%      | 优秀   |
| 算法B | 89.1% | 18.7    | 65.3%      | 良好   |
| 算法C | 85.6% | 22.1    | 58.9%      | 需改进  |
...
- ✅ **时序性能指标**: 完整的MAE、RMSE、方差、标准差等量化指标体系及评估意义详解
