...
class TimeAligner:
    ...
    def calculate_global_time_offset(self, record_data: List[Note], replay_data: List[Note]) -> float:
        ...
        # 不计算全局时间偏移，因为按键匹配算法已有动态阈值（500-2000ms）容错
        self.global_time_offset = 0.0
        
        logger.info(f"✅ 全局时间偏移量设为0 (按键匹配算法已有动态阈值容错，无需额外偏移)")
        
        # 注释掉全局偏移计算（按键匹配的动态阈值已足够容错）
        # n = min(20, len(record_times), len(replay_times))
        # offsets = [replay_times[i] - record_times[i] for i in range(n)]
        # self.global_time_offset = np.median(offsets)
        
        # 注释掉DTW算法（会产生一对多对齐，在钢琴数据中无意义）
        # total_notes = len(record_times) + len(replay_times)
        # logger.debug(f"数据量：{total_notes}个音符，使用简单的全局DTW对齐")
    ...
    # 注释掉动态时间窗口DTW方法，因为可能影响未知延迟系统的统计准确性
