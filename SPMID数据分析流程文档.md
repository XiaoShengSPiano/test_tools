...
本节统一给出各评价指标的定义、单位和数据口径，确保统计含义一致且可复现。
...
- 单个样本的带符号偏差定义为：
  ...
  $$
  x_i = \mathrm{replay\_keyon}_i - \mathrm{record\_keyon}_i
  $$
  
  （内部单位0.1ms，展示时除以10为ms）
...
- **数学公式**：
  ...
  $$
  \mu = \frac{1}{n} \sum_{i=1}^{n} x_i = \frac{1}{n} \sum_{i=1}^{n} (t_{\text{replay},i} - t_{\text{record},i})
  $$
...
- **数学公式**（本项目实现，使用绝对值）：
  ...
  $$
  \mu_y = \frac{1}{n} \sum_{i=1}^{n} y_i = \frac{1}{n} \sum_{i=1}^{n} |x_i|
  $$

- **重要说明**：本项目**使用绝对值** $y_i = |x_i|$ 计算总体方差，而非标准公式中的带符号值 $x_i$。这样做的目的是用于分析"时延与按键的关系"（关注误差大小而非提前/滞后方向）

- **数据来源**：对所有匹配对的绝对误差（即 `abs(keyon_offset)`）计算
...
- **重要说明**：本项目**使用绝对值** $y_i = |x_i|$ 计算总体标准差，与总体方差的计算方式保持一致
...
##### 6. Z-Score（标准化偏差，Standard Score）

- **定义**：将原始延时值标准化为相对于均值的标准差倍数，用于识别异常值和进行跨算法对比

- **用途**：
  
  - 识别异常延时值（通常 |Z-Score| > 3 被视为异常）
  - 在多算法模式下，将不同算法的延时值标准化到同一尺度，便于对比
  - 消除不同算法之间的量纲差异，突出相对偏差
...
- **数学公式**：
  ...
  - $x_i$：第 $i$ 个匹配对的原始延时值（带符号的 `keyon_offset`，单位：ms）
  - $\mu$：总体均值（带符号，单位：ms）
  - $\sigma$：总体标准差（单位：ms）
  - $z_i$：第 $i$ 个匹配对的 Z-Score 值（无量纲）

- **公式含义**：
  
  - $z_i = 0$：该点的延时值等于总体均值
  - $z_i > 0$：该点的延时值大于总体均值，表示延迟
  - $z_i < 0$：该点的延时值小于总体均值，表示提前
  - $|z_i| = 1$：该点的延时值距离均值 1 个标准差
  - $|z_i| = 3$：该点的延时值距离均值 3 个标准差（通常被视为异常值阈值）

- **Z-Score 参考线**：
  
  - **Z-Score = 0**：均值线，表示延时值等于总体均值
  - **Z-Score = +3**：上阈值线，表示延时值比均值大 3 个标准差（异常延迟）
  - **Z-Score = -3**：下阈值线，表示延时值比均值小 3 个标准差（异常提前）

- **数据来源**：
  
  - 使用带符号的 `keyon_offset`（即 $x_i$）计算 Z-Score
  - 使用总体均值 $\mu$ 和总体标准差 $\sigma$（基于带符号值计算）

- **单位**：Z-Score 值本身无量纲（标准化后的值）

- **代码实现**：
  
  - 单算法模式：`backend/piano_analysis_backend.py:generate_key_delay_zscore_scatter_plot()`
  - 多算法模式：`backend/multi_algorithm_plot_generator.py:generate_multi_algorithm_key_delay_zscore_scatter_plot()`

- **UI显示**：
  
  - "按键与延时Z-Score标准化散点图"：x轴为按键ID，y轴为Z-Score值
  - "锤速与延时Z-Score标准化散点图"：x轴为锤速，y轴为Z-Score值

- **应用场景**：
  
  - **异常值检测**：Z-Score 绝对值大于 3 的点通常被视为异常值，需要重点关注
  - **跨算法对比**：在多算法模式下，不同算法的延时值被标准化到同一尺度，便于直接对比不同算法的性能
  - **相对偏差分析**：Z-Score 消除了绝对量纲的影响，突出显示相对偏差，便于识别系统性偏差模式

---

##### 7. 变异系数（Coefficient of Variation, CV）

- **定义**：标准差与均值的比值，用于衡量数据的相对变异程度，消除量纲影响

- **用途**：
  
  - 评估延时数据的相对稳定性（CV 越小，数据越稳定）
  - 比较不同算法或不同数据集的变异程度
  - 识别延时波动是否在可接受范围内
...
- **数学公式**：
  ...
  - $\sigma$：总体标准差（基于带符号的 `keyon_offset` 计算，单位：ms）
  - $\mu$：总体均值（基于带符号的 `keyon_offset` 计算，单位：ms）
  - $|\mu|$：总体均值的绝对值
  - $\mathrm{CV}$：变异系数（百分比，例如 15.5 表示 15.5%）

- **公式含义**：
  
  - **CV = 0%**：标准差为 0，所有数据值完全相同（理想情况）
  - **CV < 15%**：数据相对稳定，变异程度较小
  - **CV = 15-30%**：数据中等变异程度
  - **CV > 30%**：数据变异程度较大，需要关注
  - **CV 无意义**：当均值 $\mu$ 接近 0 时，CV 可能无意义或非常大（此时返回 0.0）

- **计算说明**：
  
  - 使用总体标准差 $\sigma$（基于带符号的 `keyon_offset` $x_i$ 计算）
  - 使用总体均值 $\mu$（基于带符号的 `keyon_offset` $x_i$ 计算）
  - 使用总体均值 $\mu$ 的绝对值 $|\mu|$ 作为分母
  - 乘以 100% 转换为百分比形式
  - 如果 $|\mu| < 10^{-6}$（接近 0），则无法计算 CV，返回 0.0

- **为什么使用 $|\mu|$ 作为分母**：
  
  - 总体均值 $\mu$ 可能为正（整体延迟）或负（整体提前）
  - 使用绝对值确保 CV 始终为正数，便于解释
  - 反映相对于均值绝对值的变异程度，不受提前/滞后方向影响

- **数据来源**：
  
  - 总体标准差：使用 `get_standard_deviation()`（基于带符号的 `keyon_offset` 计算）
  - 总体均值：使用 `get_mean_error()`（基于带符号的 `keyon_offset` 计算）
  
  注意：标准差和均值都使用带符号的原始值 $x_i$（`keyon_offset`），而不是绝对值 $|x_i|$。这与 MAE、总体方差（用于"时延与按键的关系"分析）的计算方式不同。

- **单位**：百分比（%），例如 15.5 表示 15.5%

- **代码实现**：`spmid/note_matcher.py:get_coefficient_of_variation()`
...
- **UI显示**：数据概览页面的"变异系数(CV)"

- **应用场景**：
  
  - **稳定性评估**：CV 值越小，说明延时数据越稳定，波动越小
  - **算法对比**：在多算法模式下，比较不同算法的 CV 值，评估哪个算法的延时更稳定
  - **质量评估**：CV < 15% 通常表示延时质量较好，CV > 30% 可能需要优化
  - **异常检测**：如果 CV 值异常大，可能表示数据中存在大量异常值或系统性问题
...
- **变异系数（CV）**：
  ...
  其中 $\sigma$ 和 $\mu$ 都基于带符号的 `keyon_offset` 计算，分母使用 $|\mu|$ 的绝对值

**特殊说明**：

- **正态拟合曲线**：使用带符号的原始数据和样本标准差（分母 n-1），用于评估延时分布是否符合正态分布
...
从 `offset_data` 中提取 `keyon_offset`（带符号，单位 0.1ms），转换为 ms 单位（除以 10.0），保留符号：
...
使用 Plotly 的 `Histogram` 绘制，设置 `histnorm='probability density'` 进行概率密度归一化，使得直方图总面积 = 1。Plotly 自动确定区间数（bins）、统计频数、归一化并绘制。

**3. 计算统计量**

计算样本均值和样本标准差（使用 n-1 作为分母，贝塞尔校正）：
...
- **确定绘制范围**：取数据范围与 3σ 范围的并集，既覆盖实际数据，又展示理论分布特征
  
  - 在正态分布中，约 99.7% 的数据落在 $[\mu - 3\sigma, \mu + 3\sigma]$ 范围内
  - 曲线起点：$\min(\mu - 3\sigma, \min_x)$
  - 曲线终点：$\max(\mu + 3\sigma, \max_x)$

- **生成采样点**：在范围内均匀生成 200 个 x 坐标点

- **计算概率密度**：对每个 x 点，使用正态分布概率密度函数（PDF）计算 y 值
  **正态分布概率密度函数公式**：
  ...
  - $\frac{1}{\sigma\sqrt{2\pi}}$：归一化常数，确保曲线下面积为 1
  - $\exp\left(-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2\right)$：指数衰减项，表示距离均值越远，概率密度越小
  - $\frac{x-\mu}{\sigma}$：标准化偏差（z-score），表示该点距离均值有多少个标准差

- **绘制曲线**：用线段连接所有 (x, y) 点，形成连续平滑的红色曲线

**关键技术细节**：

- **使用样本标准差（n-1）**：贝塞尔校正，得到总体方差的无偏估计，更适合模型拟合
- **使用带符号数据**：评估延时分布是否符合正态分布，而非仅分析误差大小
- **取数据范围与 3σ 范围的并集**：既覆盖实际数据，又展示理论分布特征
- **200 个采样点**：平衡平滑度与计算效率
- **概率密度归一化**：直方图总面积 = 1，与正态分布的面积一致，便于对比评估
...
- 基础阈值：500（对应50毫秒）
- 持续时间因子：clamp(0.6, duration/500, 1.0)
- 最终阈值范围：300-500（对应30-50毫秒）
- 总误差：只使用keyon_offset的绝对值
