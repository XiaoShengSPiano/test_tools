...
class MultiAlgorithmPlotGenerator:
    ...
    def generate_unified_waterfall_plot(
        ...
        """
        生成瀑布图的主入口方法，根据模式分发到对应的处理方法

        Args:
            analyzers: 分析器列表
            algorithm_names: 算法名称列表
            is_multi_algorithm: 是否多算法模式
            time_filter: 时间过滤器（暂未使用）
            key_filter: 按键过滤器（暂未使用）
        ...
        if is_multi_algorithm:
            return self._generate_multi_algorithm_waterfall_plot(backend, analyzers, algorithm_names)
        else:
            return self._generate_single_algorithm_waterfall_plot(backend, analyzers[0], algorithm_names[0])

    def _generate_single_algorithm_waterfall_plot(self, backend, analyzer, algorithm_name: str) -> Any:
        """
        生成单算法模式的瀑布图
        ...
        """
        try:
            logger.info(f"📊 开始生成单算法瀑布图，算法: {algorithm_name}")

            if not analyzer:
                logger.warning(f"⚠️ 分析器 '{algorithm_name}' 为空")
                return self._create_empty_plot("分析器为空")
            ...
            try:
                ...
                logger.warning(f"⚠️ 获取平均延时失败: {e}")

            # 收集单算法数据（y_offset=0）
            algorithm_bars = self._collect_algorithm_comprehensive_data(
                analyzer, 0, algorithm_name, 0, avg_delay_ms
            )

            if not algorithm_bars:
                logger.warning("⚠️ 没有有效的数据点")
                return self._create_empty_plot("没有有效的数据点")

            # 创建单算法图表
            fig = self._create_waterfall_figure_from_bars(
                [{'analyzer': analyzer, 'bars': algorithm_bars, 'algorithm_name': algorithm_name, 'y_offset': 0}],
                is_multi_algorithm=False
            )

            logger.info(f"✅ 单算法瀑布图生成成功: {len(algorithm_bars)} 个数据点")
            return fig

        except Exception as e:
            logger.error(f"❌ 生成单算法瀑布图失败: {e}")
            return self._create_empty_plot(f"生成失败: {str(e)}")

    def _generate_multi_algorithm_waterfall_plot(self, backend, analyzers: List[Any], algorithm_names: List[str]) -> Any:
        """
        生成多算法模式的瀑布图
        ...
        """
        try:
            logger.info(f"📊 开始生成多算法瀑布图，共 {len(analyzers)} 个算法")
            ...
            # 创建多算法图表
            fig = self._create_waterfall_figure_from_bars(all_bars_by_algorithm, is_multi_algorithm=True)

            total_bars = sum(len(alg_data['bars']) for alg_data in all_bars_by_algorithm)
            logger.info(f"✅ 多算法瀑布图生成成功: {total_bars} 个数据点")
            return fig
    ...
    def _create_waterfall_figure_from_bars(self, all_bars_by_algorithm: List[Dict], is_multi_algorithm: bool) -> Any:
        """
        从bars数据创建瀑布图

        Args:
            all_bars_by_algorithm: 按算法分组的bars数据
            is_multi_algorithm: 是否多算法模式
        ...
        logger.info(f"✅ 瀑布图创建完成: 总计 {total_bars} 个bars (匹配对: {matched_bars}, 丢锤: {drop_hammer_bars}, 多锤: {multi_hammer_bars})")
        return fig
    ...
    def _collect_algorithm_comprehensive_data(self, analyzer, y_offset: float, algorithm_name: str, alg_idx: int, avg_delay_ms: float = 0.0) -> List[Dict]:
            ...
            if hasattr(note_matcher, 'match_results'):
                for result in note_matcher.match_results:
                    ...
                    try:
                        ...
                        # 检查 replay_index 是否为 None
                        if result.replay_index is None:
                            replay_note = None
                        else:
                    ...
                    # 根据匹配结果确定颜色、评级和数据类型
                    grade_name = "未知"
                    match_index = getattr(result, 'record_index', 'N/A')
                    delay_ms = 0.0
                    relative_delay_ms = 0.0

                    if result.is_success:
                        # 成功匹配：计算绝对延时（录制与播放按键keyon相减）
                        if record_note is not None and replay_note is not None:
                            # 计算录制和播放音符的keyon时间
                            record_keyon = self._calculate_note_keyon_time(record_note)
                            replay_keyon = self._calculate_note_keyon_time(replay_note)
                            # 绝对延时 = 播放keyon - 录制keyon（转换为毫秒）
                        ...
                        if delay_ms <= 20:
                            color_intensity = 0.8  # 优秀 - 深绿
                            grade_name = "优秀"
                        elif delay_ms <= 30:
                            color_intensity = 0.6  # 良好 - 中绿
                            grade_name = "良好"
                        elif delay_ms <= 50:
                            color_intensity = 0.4  # 一般 - 浅绿
                            grade_name = "一般"
                        elif delay_ms <= 1000:
                            color_intensity = 0.3  # 较差 - 黄绿
                            grade_name = "较差"
                        else:
                            color_intensity = 0.2  # 严重 - 黄
                            grade_name = "严重"
                    else:
                        color_intensity = 0.1  # 失败 - 红
                        grade_name = "失败"

                    # 添加录制数据的条形
                    logger.info(f"🎹 提取录制数据: 键位={getattr(record_note, 'id', 'N/A')}, 有锤子={hasattr(record_note, 'hammers') and record_note.hammers is not None}")
                    if hasattr(record_note, 'hammers') and record_note.hammers is not None:
                        logger.info(f"🎹 录制锤子详情: 长度={len(record_note.hammers) if hasattr(record_note.hammers, '__len__') else 'N/A'}, 有index={hasattr(record_note.hammers, 'index')}, 有values={hasattr(record_note.hammers, 'values')}")

                    record_bars = self._extract_note_bars_for_multi(record_note, 'record', y_offset, color_intensity, algorithm_name, grade_name, match_index, delay_ms, relative_delay_ms)
                    logger.info(f"🎹 录制条形提取结果: {len(record_bars)} 个条形")
                    algorithm_bars.extend(record_bars)

                    # 添加播放数据的条形（如果存在）
                    if replay_note is not None:
                        logger.info(f"🎵 提取播放数据: 键位={getattr(replay_note, 'id', 'N/A')}, 有锤子={hasattr(replay_note, 'hammers') and replay_note.hammers is not None}")
                        if hasattr(replay_note, 'hammers') and replay_note.hammers is not None:
                            logger.info(f"🎵 播放锤子详情: 长度={len(replay_note.hammers) if hasattr(replay_note.hammers, '__len__') else 'N/A'}, 有index={hasattr(replay_note.hammers, 'index')}, 有values={hasattr(replay_note.hammers, 'values')}")

                        replay_bars = self._extract_note_bars_for_multi(replay_note, 'replay', y_offset, color_intensity, algorithm_name, grade_name, match_index, delay_ms, relative_delay_ms)
                        logger.info(f"🎵 播放条形提取结果: {len(replay_bars)} 个条形")

                        # 如果有匹配的replay数据，更新record bars的hover文本以包含replay信息
                        if record_bars and replay_bars:
                            logger.info(f"🔗 开始合并匹配对信息: {len(record_bars)} 个录制条形, {len(replay_bars)} 个播放条形")
                            self._merge_matched_hover_info(record_bars, replay_bars, avg_delay_ms)
                            logger.info(f"✅ 合并完成，检查结果...")
                            # 检查合并后的结果
                            merged_count = 0
                            for bar in record_bars:
                                if '播放数据:' in bar.get('text', ''):
                                    merged_count += 1
                                    logger.info(f"✅ 合并成功 - 键位 {bar.get('original_key_id')}: 包含播放数据")
                            logger.info(f"📊 合并统计: {merged_count}/{len(record_bars)} 个录制条形包含播放数据")
                        else:
                            logger.debug(f"⚠️ 跳过合并: record_bars={bool(record_bars)}, replay_bars={bool(replay_bars)}")
                        algorithm_bars.extend(replay_bars)
        ...
        if drop_hammers:
            for error_note in drop_hammers:
                try:
                    note_index = error_note.global_index if hasattr(error_note, 'global_index') and error_note.global_index >= 0 else error_note.infos[0].index
                    if 0 <= note_index < len(initial_valid_record_data):
                        note = initial_valid_record_data[note_index]
                        if note and hasattr(note, 'hammers') and note.hammers:
                            bars = self._extract_note_bars_for_multi(note, 'record', y_offset, 0.1, algorithm_name, "失败", note_index, 0.0, 0.0, 'drop_hammer')
                            for bar in bars:
                                bar['text'] = ''
        ...
        if multi_hammers:
            for error_note in multi_hammers:
                try:
                    note_index = error_note.global_index if hasattr(error_note, 'global_index') and error_note.global_index >= 0 else error_note.infos[0].index
                    if 0 <= note_index < len(initial_valid_replay_data):
                        note = initial_valid_replay_data[note_index]
                        if note and hasattr(note, 'hammers') and note.hammers:
                            bars = self._extract_note_bars_for_multi(note, 'replay', y_offset, 0.1, algorithm_name, "失败", note_index, 0.0, 0.0, 'multi_hammer')
                            for bar in bars:
                                bar['text'] = ''
                            algorithm_bars.extend(bars)
                except:
            ...
            logger.info(f"📊 没有多锤数据")

        logger.info(f"📊 算法 '{algorithm_name}': 收集到 {len(algorithm_bars)} 个瀑布图条形")
    ...
    def _extract_note_bars_for_multi(self, note, label: str, y_offset: float, color_intensity: float, algorithm_name: str, grade_name: str = "未知", match_index: str = "N/A", delay_ms: float = 0.0, relative_delay_ms: float = 0.0, data_type: str = None) -> List[Dict]:
        ...
        # 为每个锤击创建bar段
        if hasattr(note, 'hammers') and len(note.hammers) > 0 and hasattr(note.hammers, 'index') and len(note.hammers.index) > 0:
            # 获取按键ID
            key_id = getattr(note, 'id', 1)

            # 计算第一个锤子的锤击时间
            first_hammer_time = "N/A"
            if hasattr(note, 'hammers') and len(note.hammers) > 0 and hasattr(note.hammers, 'index') and len(note.hammers.index) > 0:
                first_hammer_time = note.hammers.index[0] + getattr(note, 'offset', 0)
            ...
            if hasattr(note, 'after_touch') and note.after_touch is not None and len(note.after_touch) > 0 and hasattr(note.after_touch, 'index') and len(note.after_touch.index) > 0:
                key_off = note.after_touch.index[-1] + getattr(note, 'offset', 0)
            else:
                key_off = note.hammers.index[-1] + getattr(note, 'offset', 0) + 100

            # 为每个锤击创建bar段
            for i in range(len(note.hammers)):
                t_hammer = note.hammers.index[i] + getattr(note, 'offset', 0)

                # 提取当前锤子的力度
                hammer_velocity = "N/A"
                hammer_velocity_norm = 0.5
                if hasattr(note.hammers, 'values') and i < len(note.hammers.values):
                ...
                bar = {
                    ...
                    'first_hammer_time': first_hammer_time  # 第一个锤子的锤击时间
                }
                ...
                if label == 'record':
                    ...
                    bar['text'] = (
                        ...
                        f'等级: {grade_name}<br>'
                        f'索引: {match_index}<br>'
                        f'首锤时间: {first_hammer_time} ({first_hammer_time/10:.2f}ms)<br>'
                        f'按键按下: {t_hammer/10:.2f}ms<br>'
                        f'按键释放: {key_off/10:.2f}ms<br>'
                    )
                else:
        ...
        try:
            ...
            for algorithm in ready_algorithms:
                ...
                try:
                    ...
                    matched_pairs = algorithm.analyzer.note_matcher.get_matched_pairs() if hasattr(algorithm.analyzer, 'note_matcher') and algorithm.analyzer.note_matcher else []
        ...
        try:
                ...
                try:
                    ...
                    for item in offset_data:
                        ...
                        try:
                            ...
                            customdata_list.append([record_index, replay_index, key_id_int, delay_ms, descriptive_name])
                        except (ValueError, TypeError):
                            continue
            ...
            for alg_data in algorithm_data_list:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    showlegend=True,
                    hovertemplate=f"算法: {alg_data['descriptive_name']}<br>键位: %{{x}}<br>相对延时: %{{y:.2f}}ms<br>绝对延时: %{{customdata[3]:.2f}}ms<extra></extra>"
                ))
            ...
            for alg_data in algorithm_data_list:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    showlegend=True,
                    hovertemplate=f"算法: {alg_data['name']}<br>相对延时平均值 = 0ms<extra></extra>"
                ))
